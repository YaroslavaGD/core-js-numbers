...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(scalarMult / (lengthV1 * lengthV2));
}
...
function toNumber(value, def) {
  ...
  return Number.isInteger(res) ? res : def;
}
...
function getFibonacciNumber(index) {
  const sqrt5 = 5 ** 0.5;
  const a = (1 + sqrt5) ** index;
  const b = (1 - sqrt5) ** index;

  return Math.round((a - b) / (2 ** index * sqrt5));
}
...
function getCountOfOddNumbers(number) {
  ...
  for (let i = step; Math.abs(i) <= Math.abs(number); i += step) {
    if (i % 2 !== 0) sum += 1;
  }
